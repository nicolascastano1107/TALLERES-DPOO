...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        for (Avion av : this.getAviones()) {
            if (av != null && av.getNombre().equals(nombreAvion)) {
                avionEncontrado = av;
                break;
            }
        }

        if (ruta != null && avionEncontrado != null) {
            this.vuelos.add(new Vuelo(ruta, fecha, avionEncontrado));
        } else {
            throw new Exception("El código de ruta o el nombre del avión es inválido.");
        }
    }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        // TODO Implementar el método
    	  if (cantidad <= 0) throw new Exception("La cantidad debe ser mayor a 0.");
    	    ...
    	    CalculadoraTarifas calculadora = (mes.equals("01") || mes.equals("07"))
    	            ? new CalculadoraTarifasTemporadaBaja()
    	            : new CalculadoraTarifasTemporadaAlta();

    	  
    	    int resultado = vuelo.venderTiquetes(cliente, calculadora, cantidad);
